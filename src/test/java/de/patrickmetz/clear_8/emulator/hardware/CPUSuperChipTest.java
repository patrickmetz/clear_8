...
class CPUSuperChipImplTest {
    ...
    /**
     * @see CPUSuperChipImpl#opcode8XY5
     */
    @Test
    void opcode8XY5HandlesMinByteValue() {
        registers.write(0x0, 0x01);
        registers.write(0x1, 0x01);

        writeOpcodeToMemory(0x8015); // X = 0x0, Y = 0x1
        processOpcode();

        assertEquals(0x0, registers.read(0x0)); // X = 0?
        assertEquals(0x1, registers.read(CARRY)); // no borrow?
    }
     ...
     */
    @Test
    void opcode8XY5HandlesUnderflow() {
        registers.write(0x0, 0x00);
        registers.write(0x1, 0x01);
    ...
    /**
     * @see CPUSuperChipImpl#opcode8XY7
     */
    @Test
    void opcode8XY7HandlesMinByteValue() {
        registers.write(0x0, 0x01);
        registers.write(0x1, 0x01);

        writeOpcodeToMemory(0x8017); // X = 0x0, Y = 0x1
        processOpcode();

        assertEquals(0x0, registers.read(0x0)); // X = (Y - X))?
        assertEquals(0x1, registers.read(CARRY)); // borrow not set?
    }
     ...
     */
    @Test
    void opcode8XY7HandlesUnderflow() {
        registers.write(0x0, 0x01);
        registers.write(0x1, 0x00);
    ...
    /**
     * @see CPUSuperChipImpl#opcode8XYE
     */
    @Test
    void opcode8XYE() {
        registers.write(0x0, 0xFF);
